    ...
    <Listbox value={selected} onChange={handleSelect}>
      <div className="relative z-10 mt-1">
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute w-full md:w-52 py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {items.map((item) => (
              <Listbox.Option
                key={item.id}
                className={({ active }) =>
